...
public class Result<T> extends net.authorize.xml.Result<T> {
	...
	public void printMessages() {
		...
		Vector resultVector = new Vector();
			...
			System.out.println("Result customerProfile Id: " + getCustomerPaymentProfile().getCustomerPaymentProfileId());
                        System.out.println("Result Masked Payment Card : "+  pp.getPaymentList().get(0).getCreditCard().getCreditCardNumber());
                ...
                if(resultCode == null){
                    ...
                    System.out.println("Result Code: " + (resultCode != null ? resultCode : "No result code"));
                        ...
                        if(resultCode.equalsIgnoreCase("Ok")){
                            String transResponseCode = getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.RESPONSE_CODE);
                            int respcd = Integer.parseInt(transResponseCode);
                            if(respcd==1){
                                resultVector.add("00");
                                resultVector.add("Transaction Accepted");
                                resultVector.add(getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.RESPONSE_REASON_TEXT));
                                String inVoiceNumber = getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.INVOICE_NUMBER);
                                String authCode = getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.AUTHORIZATION_CODE);
                                //splitTenderId = result.getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.SPLIT_TENDER_ID);
                                String transactionId = getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.TRANSACTION_ID);
                                ...
                                resultVector.add(transactionId);
                                 com.server.Constants.logger.info("Invoice Number : "+inVoiceNumber+" Authentication Code : "+authCode+" Transaction ID : "+transactionId);
                            }else if(respcd==2){
                                resultVector.add("01");
