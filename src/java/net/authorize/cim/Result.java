...
public class Result<T> extends net.authorize.xml.Result<T> {
    ...
    private void importResponseMessages(Transaction txn) {
        NodeList messages_list = txn.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_MESSAGES.getFieldName());
        ...
        String customerProfileId = getElementText(txn.getCurrentResponse().getDocumentElement(),
                AuthNetField.ELEMENT_CUSTOMER_PROFILE_ID.getFieldName());
    ...
    private void importCustomerShippingAddressId(Transaction txn) {
        String customerShippingAddress = getElementText(
                ...
                AuthNetField.ELEMENT_CUSTOMER_ADDRESS_ID.getFieldName());
    ...
    private void importCustomerPaymentProfileId(Transaction txn) {
        NodeList payment_profile_id_list = txn.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_CUSTOMER_PAYMENT_PROFILE_ID_LIST.getFieldName());
    ...
    private void importCustomerShippingAddressIdList(Transaction txn) {
        NodeList shipping_address_id_list = txn.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_CUSTOMER_SHIPPING_ADDRESS_ID_LIST.getFieldName());
    ...
    private void importCustomerProfileIdList(Transaction txn) {
        NodeList profile_id_list = txn.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_IDS.getFieldName());
    ...
    private void importCustomerProfile(Transaction transaction) {
        NodeList profile_list = transaction.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_PROFILE.getFieldName());
        ...
        customerProfile.setMerchantCustomerId(getElementText(profile_el,
                AuthNetField.ELEMENT_MERCHANT_CUSTOMER_ID.getFieldName()));
        ...
        customerProfile.setCustomerProfileId(getElementText(profile_el,
                AuthNetField.ELEMENT_CUSTOMER_PROFILE_ID.getFieldName()));
        // payment profiles
    ...
    private void importShippingAddress(Transaction transaction) {
        NodeList address_list = transaction.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_ADDRESS.getFieldName());
        ...
        shipToAddress.setAddressId(getElementText(address_el,
                AuthNetField.ELEMENT_CUSTOMER_SHIPPING_ADDRESS_ID.getFieldName()));
    ...
    private void importShipToList(Element root_el, CustomerProfile customerProfile) {
        ...
        for (int i = 0; i < ship_to_list.getLength(); i++) {
            ...
            shipToAddress.setAddressId(getElementText(ship_to_el,
                    AuthNetField.ELEMENT_CUSTOMER_ADDRESS_ID.getFieldName()));
            customerProfile.addShipToAddress(shipToAddress);
        }
    }
    ...
    private void importPaymentProfiles(Element root_el) {
        NodeList payment_profile_list = root_el.getElementsByTagName(AuthNetField.ELEMENT_PAYMENT_PROFILES
                .getFieldName());
        ...
        for (int i = 0; i < payment_profile_list.getLength(); i++) {
            ...
            paymentProfile.setCustomerType(CustomerType.findByName(getElementText(payment_profile_el,
                    AuthNetField.ELEMENT_CUSTOMER_TYPE.getFieldName())));
            importBillTo(payment_profile_el, paymentProfile);
            paymentProfile.setCustomerPaymentProfileId(getElementText(payment_profile_el,
                    AuthNetField.ELEMENT_CUSTOMER_PAYMENT_PROFILE_ID.getFieldName()));
    ...
    private void importCustomerPaymentProfile(Transaction transaction) {
        NodeList payment_profile_list = transaction.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_PAYMENT_PROFILE.getFieldName());
    ...
    private void importPaymentInfo(Element root_el, PaymentProfile paymentProfile) {
        ...
        if (credit_card_list.getLength() != 0) {
            ...
            creditCard.setMaskedCreditCardNumber(getElementText(credit_card_el,
                    AuthNetField.ELEMENT_CREDIT_CARD_NUMBER.getFieldName()));
            creditCard.setExpirationDate(getElementText(credit_card_el,
                    AuthNetField.ELEMENT_CREDIT_CARD_EXPIRY.getFieldName()));
        ...
        if (bank_account_list.getLength() != 0) {
            ...
            bankAccount.setRoutingNumber(getElementText(bank_account_el,
                    AuthNetField.ELEMENT_ROUTING_NUMBER.getFieldName()));
            bankAccount.setBankAccountNumber(getElementText(bank_account_el,
                    AuthNetField.ELEMENT_ACCOUNT_NUMBER.getFieldName()));
            bankAccount.setBankAccountName(getElementText(bank_account_el,
                    AuthNetField.ELEMENT_NAME_ON_ACCOUNT.getFieldName()));
    ...
    private void importDirectResponse(Transaction txn) {
        NodeList validation_direct_response_list = txn.getCurrentResponse().getDocument()
                .getElementsByTagName(AuthNetField.ELEMENT_VALIDATION_DIRECT_RESPONSE_LIST.getFieldName());
    ...
    public Vector returnCreateProfile() {
        Vector resultVector = new Vector();
        System.out.println("Result Code Vaue: " + (resultCode != null ? resultCode : "No result code"));
        if (resultCode == null) {
            resultVector.add("01");
            resultVector.add("No Result Code");
        } else if (getCustomerProfileId() == null) {
            resultVector.add("01");
            resultVector.add("Customer Profile Not Created");
            for (int i = 0; i < messages.size(); i++) {
                Message message = (Message) messages.get(i);
                System.out.println(message.getCode() + " - " + message.getText());
                com.server.Constants.logger.info("*****Response From Authorize server********" + "\n"
                        + message.getCode() + " - " + message.getText());
                resultVector.add(message.getText());
            }
        } else {
            resultVector.add("00");
            resultVector.add(getCustomerProfileId());
            System.out.println("Result customerProfile Id: " + getCustomerProfileId());
            for (int i = 0; i < messages.size(); i++) {
                Message message = (Message) messages.get(i);
                com.server.Constants.logger.info("*****Response From Authorize server********" + "\n"
    ...
    public Vector printCustomerProfileMessages() {
        Vector resultVector = new Vector();
        if (resultCode == null) {
            resultVector.add("01");
            resultVector.add("No Result Code");
            System.out.println("Result Code: " + (resultCode != null ? resultCode : "No result code"));
        } else if (getCustomerPaymentProfile() == null) {
            resultVector.add("01");
            resultVector.add("Customer Payment Profile Could not be fetched");
            for (int i = 0; i < messages.size(); i++) {
                Message message = (Message) messages.get(i);
                System.out.println(message.getCode() + " - " + message.getText());
                resultVector.add(message.getText());
            ...
            PaymentProfile pp = getCustomerPaymentProfile();
            System.out.println("Result customerProfile Id: "
                    + getCustomerPaymentProfile().getCustomerPaymentProfileId());
            System.out.println("Result Masked Payment Card : "
                    + pp.getPaymentList().get(0).getCreditCard().getCreditCardNumber());
            // System.out.println("Result Payment Card : "+ pp.getPaymentList().get(0).getCreditCard());
            resultVector.add("00");
            for (int i = 0; i < messages.size(); i++) {
                Message message = (Message) messages.get(i);
                System.out.println(message.getCode() + " - " + message.getText());
            ...
            resultVector.add(getCustomerPaymentProfile().getCustomerPaymentProfileId());
            resultVector.add(pp.getPaymentList().get(0).getCreditCard().getCreditCardNumber());
    ...
    public Vector printTransactionProfileMessages() {
        Vector resultVector = new Vector();
        if (resultCode == null) {
            resultVector.add("01");
            resultVector.add("No Result Code");
            ...
            if (resultCode.equalsIgnoreCase("Ok")) {
                String transResponseCode = getDirectResponseList().get(0).getDirectResponseMap()
                        .get(ResponseField.RESPONSE_CODE);
                int respcd = Integer.parseInt(transResponseCode);
                if (respcd == 1) {
                    resultVector.add("00");
                    resultVector.add("Transaction Accepted");
                    resultVector.add(getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.RESPONSE_REASON_TEXT));
                    String inVoiceNumber = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.INVOICE_NUMBER);
                    String authCode = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.AUTHORIZATION_CODE);
                    // splitTenderId =
                    // result.getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.SPLIT_TENDER_ID);
                    String transactionId = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.TRANSACTION_ID);
                    // resultVector.add(getDirectResponseList().get(0).getDirectResponseMap().get(ResponseField.TRANSACTION_ID));
                    resultVector.add(inVoiceNumber);
                    resultVector.add(authCode);
                    resultVector.add(transactionId);
                    com.server.Constants.logger.info("Invoice Number : " + inVoiceNumber + " Authentication Code : "
                            + authCode + " Transaction ID : " + transactionId);
                } else if (respcd == 2) {
                    resultVector.add("01");
                    resultVector.add("Transaction Declined");
                    resultVector.add(getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.RESPONSE_REASON_TEXT));
                    String inVoiceNumber = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.INVOICE_NUMBER);
                    ...
                    resultVector.add("02");
                    resultVector.add("Error during Transaction");
                    resultVector.add(getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.RESPONSE_REASON_TEXT));
                    String inVoiceNumber = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.INVOICE_NUMBER);
                    ...
                    resultVector.add("03");
                    resultVector.add("Transaction held for review");
                    resultVector.add(getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.RESPONSE_REASON_TEXT));
                    String inVoiceNumber = getDirectResponseList().get(0).getDirectResponseMap()
                            .get(ResponseField.INVOICE_NUMBER);
                ...
                resultVector.add("01");
                resultVector.add("Error in Transaction");
                for (int i = 0; i < messages.size(); i++) {
                    Message message = (Message) messages.get(i);
                    // System.out.println(message.getCode() + " - " + message.getText());
                    resultVector.add(message.getText());
    ...
    public Vector returnDeleteProfile() {
        Vector resultVector = new Vector();
        System.out.println("Result Code Vaue: " + (resultCode != null ? resultCode : "No result code"));
        if (resultCode == null) {
            resultVector.add("01");
            ...
            if (resultCode.equalsIgnoreCase("Ok")) {
                resultVector.add("00");
                resultVector.add("Profile Deleted Successfully");
                for (int i = 0; i < messages.size(); i++) {
                    Message message = (Message) messages.get(i);
                    System.out.println(message.getCode() + " - " + message.getText());
                    resultVector.add(message.getText());
