...
public class Transaction extends net.authorize.Transaction {
    ...
    private ValidationModeType validationMode = ValidationModeType.LIVE_MODE;
    ...
    public static final Transaction createTransaction(Transaction transaction, BasicXmlDocument response) {
        ...
        // System.out.println(transaction.getPaymentProfileList());
        return transaction;
    }
    ...
    private void addCustomerProfileId(BasicXmlDocument document) {
        if (customerProfile != null && customerProfile.getCustomerProfileId() != null) {
            Element customer_profile_id_el = document.createElement(AuthNetField.ELEMENT_CUSTOMER_PROFILE_ID
                    .getFieldName());
            customer_profile_id_el.appendChild(document.getDocument().createTextNode(
    ...
    private void addCustomerAddressId(BasicXmlDocument document) {
        if (this.paymentTransaction.getCustomerShippingAddressId() != null) {
            Element customer_shipping_address_id_el = document.createElement(AuthNetField.ELEMENT_CUSTOMER_ADDRESS_ID
                    .getFieldName());
            customer_shipping_address_id_el.appendChild(document.getDocument().createTextNode(
                    this.paymentTransaction.getCustomerShippingAddressId()));
    ...
    private void addCustomerShippingAddressId(BasicXmlDocument document) {
        if (this.paymentTransaction.getCustomerShippingAddressId() != null) {
            Element customer_shipping_address_id_el = document
                    .createElement(AuthNetField.ELEMENT_CUSTOMER_SHIPPING_ADDRESS_ID.getFieldName());
            customer_shipping_address_id_el.appendChild(document.getDocument().createTextNode(
                    this.paymentTransaction.getCustomerShippingAddressId()));
            document.getDocumentElement().appendChild(customer_shipping_address_id_el);
        }
    ...
    private void addCustomerPaymentProfileId(BasicXmlDocument document) {
        if (this.paymentTransaction.getCustomerPaymentProfileId() != null) {
            Element customer_payment_profile_id_el = document
                    .createElement(AuthNetField.ELEMENT_CUSTOMER_PAYMENT_PROFILE_ID.getFieldName());
            customer_payment_profile_id_el.appendChild(document.getDocument().createTextNode(
                    this.paymentTransaction.getCustomerPaymentProfileId()));
    ...
    private void addCustomerProfile(BasicXmlDocument document) {
        if (customerProfile != null) {
            ...
            // merchantCustomerId
            Element merchant_customer_id_el = document.createElement(AuthNetField.ELEMENT_MERCHANT_CUSTOMER_ID
                    .getFieldName());
            merchant_customer_id_el.appendChild(document.getDocument().createTextNode(
                    this.customerProfile.getMerchantCustomerId()));
            ...
            if (!StringUtils.isEmpty(this.customerProfile.getCustomerProfileId())) {
                Element customer_profile_id_el = document.createElement(AuthNetField.ELEMENT_CUSTOMER_PROFILE_ID
                        .getFieldName());
                customer_profile_id_el.appendChild(document.getDocument().createTextNode(
                        this.customerProfile.getCustomerProfileId()));
            ...
            addAddress(document, AuthNetField.ELEMENT_SHIP_TO.getFieldName(), this.customerProfile.getShipToAddress(),
     ...
     * @param profile_el
     *            - if null handles the singular element case
     */
    private void addPaymentProfiles(BasicXmlDocument document, Element profile_el) {
        if (this.paymentProfileList != null) {
            ...
            for (PaymentProfile paymentProfile : this.paymentProfileList) {
                ...
                cutomer_type_el.appendChild(document.getDocument().createTextNode(
                        paymentProfile.getCustomerType().name().toLowerCase()));
    ...
    private void addPayment(BasicXmlDocument document, Payment payment,
        ...
        if (credit_card != null) {
            ...
            cc_exp_el.appendChild(document.getDocument().createTextNode(
                    net.authorize.util.DateUtil.getFormattedDate(credit_card.getExpirationDate(),
            ...
            if (bank_account.getBankAccountType() != null) {
                ...
                account_type_el.appendChild(document.getDocument().createTextNode(
                        bank_account.getBankAccountType().getValue().toLowerCase()));
            ...
            if (bank_account.getECheckType() != null) {
                ...
                echeck_type_el.appendChild(document.getDocument().createTextNode(
                        bank_account.getECheckType().getValue()));
     ...
     * @param address
     * @param parent_el
     *            - can be null if adding directly to the document
     */
    ...
    private void addPaymentTransaction(BasicXmlDocument document) {
        if (this.paymentTransaction != null) {
            boolean authOrCaptureTxn = (net.authorize.TransactionType.AUTH_ONLY.equals(this.paymentTransaction
                    .getTransactionType()) ||
            ...
            // boolean creditTxn =
            // (net.authorize.TransactionType.CREDIT.equals(this.paymentTransaction.getTransactionType()) ||
            ...
            if (order != null && order.getTotalAmount() != null) {
                ...
                amount_el.appendChild(document.getDocument().createTextNode(
                        order.getTotalAmount().setScale(Transaction.CURRENCY_DECIMAL_PLACES, BigDecimal.ROUND_HALF_UP)
                                .toPlainString()));
                ...
                if (shippingCharges != null) {
                    ...
                    Element tax_description_el = document
                    ...
                    Element shipping_description_el = document.createElement(AuthNetField.ELEMENT_DESCRIPTION
                            .getFieldName());
                    if (shippingCharges.getFreightAmount() != null) {
                        shipping_amount_el.appendChild(document.getDocument().createTextNode(
                                shippingCharges.getFreightAmount()
                                        .setScale(Transaction.CURRENCY_DECIMAL_PLACES, BigDecimal.ROUND_HALF_UP)
                                        .toPlainString()));
                    }
                    ...
                    for (OrderItem orderItem : order.getOrderItems()) {
                        ...
                        Element description_el = document
                                .createElement(AuthNetField.ELEMENT_DESCRIPTION.getFieldName());
                        description_el.appendChild(document.getDocument()
                                .createTextNode(orderItem.getItemDescription()));

                        Element quantity_el = document.createElement(AuthNetField.ELEMENT_QUANTITY.getFieldName());
                        quantity_el.appendChild(document.getDocument().createTextNode(
                                orderItem.getItemQuantity().toBigInteger().toString()));
                        ...
                        taxable_el.appendChild(document.getDocument().createTextNode(
                                orderItem.isItemTaxable() ? TRUE.toLowerCase() : FALSE.toLowerCase()));
            ...
            if (!StringUtils.isEmpty(this.paymentTransaction.getCreditCardNumberMasked())) {
                Element credit_card_num_mask_el = document.createElement(AuthNetField.ELEMENT_CREDIT_CARD_NUMBER_MASKED
                        .getFieldName());
                credit_card_num_mask_el.appendChild(document.getDocument().createTextNode(
                        this.paymentTransaction.getCreditCardNumberMasked()));
            ...
            // bankAccountNumberMasked
                ...
                Element bank_routing_num_mask_el = document
                        .createElement(AuthNetField.ELEMENT_BANK_ROUTING_NUMBER_MASKED.getFieldName());
                bank_routing_num_mask_el.appendChild(document.getDocument().createTextNode(
                        this.paymentTransaction.getBankRoutingNumberMasked()));
                profile_trans_x_el.appendChild(bank_routing_num_mask_el);

                Element bank_acct_num_mask_el = document.createElement(AuthNetField.ELEMENT_BANK_ACCOUNT_NUMBER_MASKED
                        .getFieldName());
                bank_acct_num_mask_el.appendChild(document.getDocument().createTextNode(
                        this.paymentTransaction.getBankAccountNumberMasked()));
            ...
            if (!net.authorize.TransactionType.PRIOR_AUTH_CAPTURE.equals(this.paymentTransaction.getTransactionType())) {
                ...
                if (order != null) {
                    ...
                    Element invoice_number_el = document.createElement(AuthNetField.ELEMENT_INVOICE_NUMBER
                            .getFieldName());
                    Element description_el = document.createElement(AuthNetField.ELEMENT_DESCRIPTION.getFieldName());
                    Element purchase_order_number_el = document
                            .createElement(AuthNetField.ELEMENT_PURCHASE_ORDER_NUMBER.getFieldName());
                    ...
                    if (shippingCharges != null) {
                        purchase_order_number_el.appendChild(document.getDocument().createTextNode(
                                shippingCharges.getPurchaseOrderNumber()));
                    }
                ...
                if (authOrCaptureTxn) {
                    // recurring billing
                    Element recurring_billing_el = document.createElement(AuthNetField.ELEMENT_RECURRING_BILLING
                            .getFieldName());
                    ...
                    if (!StringUtils.isEmpty(this.paymentTransaction.getCardCode())) {
                        ...
                        card_code_el.appendChild(document.getDocument().createTextNode(
                                this.paymentTransaction.getCardCode()));
                ...
                if (!StringUtils.isEmpty(this.paymentTransaction.getSplitTenderId())) {
                    Element split_tender_id_el = document.createElement(AuthNetField.ELEMENT_SPLIT_TENDER_ID
                            .getFieldName());
                    split_tender_id_el.appendChild(document.getDocument().createTextNode(
                            this.paymentTransaction.getSplitTenderId()));
            ...
            if (!StringUtils.isEmpty(this.paymentTransaction.getTransactionId())) {
                ...
                trans_id_el.appendChild(document.getDocument().createTextNode(
                        this.paymentTransaction.getTransactionId()));
            ...
            if (!StringUtils.isEmpty(this.paymentTransaction.getApprovalCode())) {
                ...
                approval_code_el.appendChild(document.getDocument().createTextNode(
                        this.paymentTransaction.getApprovalCode()));
    ...
    private void addSplitTenderInfo(BasicXmlDocument document) {
        if (this.paymentTransaction != null) {
            ...
            if (this.paymentTransaction.getSplitTenderStatus() != null) {
                Element split_tender_status_el = document.createElement(AuthNetField.ELEMENT_SPLIT_TENDER_STATUS
                        .getFieldName());
    ...
    /**
     * @param paymentProfileList
     *            the paymentProfileList to set
     */
