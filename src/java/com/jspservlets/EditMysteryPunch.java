...
public class EditMysteryPunch extends HttpServlet {
    ...
    private static final long serialVersionUID = 3022186367749334950L;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            rs = null;
            try {
                String query = "Select punch_card.punch_card_id,punch_card.business_userid from punch_card,business_users where punch_card.business_userid=business_users.business_userid and business_users.email_id='"
                        + username + "'";
            ...
            try {
                String query = "Select count(*) from mystery_punch where punch_card_id = " + punch_card_id
                        + " and mysterypunchvalid='Y'";
                rs = stmt.executeQuery(query);
                com.server.Constants.logger.info("The select query is " + query);
            ...
            updateMysteryPunches(punch_card_id, mysteryid, mystery_punch_values, mystery_punch_probability,
                    isvalidmystery, isPaidPunchMystery, no_of_mystery_punches, (int) valid_mystery_punches);
             ...
             * out.println("<title>Servlet edit_mystery_punch</title>"); out.println("</head>"); out.println("<body>");
        ...
        if (no_of_punches_passed >= no_of_valid_punchesdb) {
            ...
            try {
                ...
                for (int i = 0; i < no_of_valid_punchesdb; i++) {
                    String updateMysteryQuery = "Update mystery_punch set value_of_myst_punch='" + mysteryValues[i]
                            + "', probability=" + mysteryProb[i] + ", paidpunchmystery='" + isMysteryPunch[i]
                            + "', mysterypunchvalid='" + isValidMystery[i] + "' where mystery_id=" + mysteryId[i] + ";";
