    ...
    public boolean userVerified(String bususerid,String email_id) throws SQLException, ServletException{
        ...
        boolean fl = true;
           try{
               db = new DBConnection();
               st = db.stmt;
               String query = "Update business_users set isemailverified = 'Y' where business_userid ="+bususerid+" and email_id='"+email_id+"'" ;
               com.server.Constants.logger.info("The query is "+query);
               System.out.println("The query is "+query);
...
//                              Constants.logger.info("Closing  Statement ");
   ...
   /*                     if(rs != null) {
     ...
     public void createFlyerPdf(String filename) throws IOException, DocumentException {
        ...
        PdfContentByte canvas = writer.getDirectContent();
           ...
           table.addCell(getDataCell(blankcell));
            ...
            String text_two = "Pay the Cashier $" + sp_of_PunchCard + " and receive "+no_of_punches_per_card+" $"+value_of_each_punch+" vouchers of store credit ($"+cost_price+" value)";
            table.addCell(getDataCell(text_two));

            table.addCell(getDataCell(blankcell));
            String text_three = "Every time you visit us, open the PaidPunch app and use a $"+value_of_each_punch+" PaidPunch towards your purchase";
    ...
    public void drawImageFlyer(PdfContentByte canvas) throws DocumentException, IOException {
        // get the image
    	//Image img = Image.getInstance(String.format("C:\\Documents and Settings\\admin\\My Documents\\NetBeansProjects\\trial\\src\\java\\com\\trial\\flyer_bg.png"));
        Image img = Image.getInstance(String.format(images_path+"flyer_bg12.png"));
        img.scalePercent(110);
       // img.scaleToFit(650, 500);
        //img.setAbsolutePosition((PageSize.A4.getHeight() - img.getScaledWidth()) / 2, (PageSize.A4.getWidth() - img.getScaledHeight()) / 2);
...
//       img1.scaleToFit(33,33);
 ...
 //      img1.scaleToFit(35,35);
    ...
    public PdfPCell getBusinessCell() throws BadElementException, MalformedURLException, IOException, DocumentException {
        ...
        Paragraph title11 = new Paragraph("Save ", FontFactory.getFont(FontFactory.HELVETICA, 32, Font.BOLD, new Color(244, 123, 39)));
        title11.setAlignment("right");


        title11.add(new Chunk(disc + "%", FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(129, 130, 133))));

        title11.add(new Chunk(" at ",FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(244, 123, 39))));


        title11.add(new Chunk(bussname, FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(129, 130, 133))));
        ...
        line2.add(new Chunk(" with",FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(244, 123, 39))));
        line2.add(new Chunk(" Paid",FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(129, 130, 133))));
        line2.add(new Chunk("Punch",FontFactory.getFont(FontFactory.HELVETICA, 32, Font.NORMAL, new Color(244, 123, 39))));
    ...
    public PdfPCell getDataCell(String cell_text) throws BadElementException, MalformedURLException, IOException, DocumentException {
        PdfPCell cell = new PdfPCell();
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setUseDescender(true);
        Paragraph title11 = new Paragraph(cell_text, FontFactory.getFont(FontFactory.HELVETICA, 12, Font.NORMAL, new Color(90,90,90)));
        title11.setAlignment("left");
        cell.addElement(title11);
        return cell;
    }

    public PdfPCell getOffCell(String cell_text) throws BadElementException, MalformedURLException, IOException, DocumentException {
        PdfPCell cell = new PdfPCell();
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setUseDescender(true);
        //Paragraph title11 = new Paragraph(cell_text, FontFactory.getFont(FontFactory.HELVETICA, 18, Font.BOLD, new Color(244, 123, 39)));

        Paragraph title11 = new Paragraph(cell_text,   FontFactory.getFont(FontFactory.HELVETICA, 16, Font.BOLD, new Color(244, 123, 39)));
        title11.setAlignment("center");

     //   title11.add(new Chunk("\nOff",FontFactory.getFont(FontFactory.HELVETICA, 18, Font.NORMAL, new Color(244, 123, 39))));
        cell.addElement(title11);
        return cell;
    }
 ...
 //       img1 = Image.getInstance("http://api.qrserver.com/v1/create-qr-code/?data="+orange_qrcode_value+"&amp;size=100x100");
    ...
    public PdfPCell getAttentionText(String cell_text) throws BadElementException, MalformedURLException, IOException, DocumentException {
        PdfPCell cell = new PdfPCell();
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setUseDescender(true);
        //Paragraph title11 = new Paragraph(cell_text, FontFactory.getFont(FontFactory.HELVETICA, 18, Font.BOLD, new Color(244, 123, 39)));

        Paragraph title11 = new Paragraph("ATTENTION : ", FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, new Color(90, 90, 90)));
        title11.setAlignment("center");
        title11.add(new Chunk("Keep this QR code hidden and allow the customer to scan it",FontFactory.getFont(FontFactory.HELVETICA, 8, Font.NORMAL, new Color(90,90,90))));
        title11.add(new Chunk(" only after receiving payment.\n",FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, new Color(90, 90, 90))));
        title11.add(new Chunk("Scanning this code will issue a PaidPunch Card to the customer.",FontFactory.getFont(FontFactory.HELVETICA, 8, Font.NORMAL, new Color(90,90,90))));
     //   title11.add(new Chunk("\nOff",FontFactory.getFont(FontFactory.HELVETICA, 18, Font.NORMAL, new Color(244, 123, 39))));
        cell.addElement(title11);
        return cell;
    }

    public PdfPCell getIssuingText(String cell_text) throws BadElementException, MalformedURLException, IOException, DocumentException {
        PdfPCell cell = new PdfPCell();
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setUseDescender(true);
        //Paragraph title11 = new Paragraph(cell_text, FontFactory.getFont(FontFactory.HELVETICA, 18, Font.BOLD, new Color(244, 123, 39)));

        Paragraph title11 = new Paragraph(cell_text,   FontFactory.getFont(FontFactory.HELVETICA, 15, Font.BOLD, new Color(255, 255, 255)));
        title11.setAlignment("left");
